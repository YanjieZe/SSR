# this is for the final leaderboard's inference
...
def benchmark(args):
    ...
    args.train_set = "test_seq.lst"
    print("testing on", args.train_set)
    valid_test_dataset = dataset.NovaDatasetV2(args)
    ...
    save_pred_root = 'data/test_seq_pred'
    if not os.path.exists(save_pred_root):
        os.mkdir(save_pred_root)
    for idx, (x, y_gt, filename) in enumerate(loop):
        ps, rs, f1s, seq_gt, pred = evaluate(model, args, x, y_gt)
        p_sum += np.sum(ps)
        r_sum += np.sum(rs)
        f1_sum += np.sum(f1s)
        
        file_name = filename[0].split('/')[-1].replace('.bpseq', '.st')
        file_name = os.path.join(save_pred_root, file_name)
        with open(file_name, 'w') as tmp_f:
            tmp_f.write(seq_gt )
            tmp_f.write('\n')
            tmp_f.write(pred)
            print("prediction is saved in ", file_name)
...
def evaluate(model, args, x, y):
    if (args.model == 'e2efold'):
        ...
        print('--------------------------------')
        return ps, rs, f1s, seqs[0], utils.pairs2dot_bracket(bps[0])
