...
#? filename(str), sequence(str), pairing information(tensor)
#! p[0]==0 when structure_is_known, why?
#* Change: p[0] fixed to 0. o.w. an error is raised.
class BPseqDataset(Dataset):
        ...
        with open(bpseq_list) as f:
            for l in f:
                ...
                if len(l)==1:
                    ...
                    raise NotImplementedError(f"Original: read_pdb() in mxfold2. Fasta is not implemented here: {bpseq_list}")
    ...
    def read(self, filename):
            ...
            for l in f:
                if not l.startswith('#'):
                    ...
                    if len(l) == 3:
                        ...
                        #! pair should always be non-negative integers, isn't it?
                        pos = 'x.<>|'.find(pair)
                        if pos >= 0:
                            # idx, pair = int(idx), -pos
                            raise NotImplementedError(f"pos >= 0 is not implemented here. : {filename}")
                        else:
                        ...
                        raise NotImplementedError(f"len(l) == 4 is not implemented here : {filename}")
                    else:
                        raise RuntimeError('invalid format: {}'.format(filename))
        ...
        if structure_is_known:
            ...
            # p.pop(0)
            # return (filename, seq, torch.tensor(p))
            raise NotImplementedError(f"structure_is_known == False is not implemented here. : {filename}")
