...
typedef typename py::array_t<int, py::array::c_style | py::array::forcecast> py_cdarray_t;

std::string predict(std::string& seq);

py::array_t<int> _linearfold(std::string& seq) {
    auto str = predict(seq);
    ...
    return py::array_t<int>(py::cast(pairs));
}

PYBIND11_MODULE(pylinearfold, m) {
    m.doc() = "LinearFold python interface";

    m.def("predict", &_linearfold, "Predict by LinearFold and convert dot-bracket representation to pair-index representation.");
}

#include "src/LinearFold.h"
#include "src/Utils/utility.h"
#include "src/Utils/utility_v.h"
#include "src/LinearFoldEval.cpp" // adding eval mode
...
#endif
    // if (!zuker_subopt){
    //     printf("%s (%.2f)\n", result.structure.c_str(), printscore);    //* result.structure.c_str() stores the final structure
    // }
