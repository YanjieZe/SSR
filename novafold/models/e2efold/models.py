...
import math
from .common.utils import soft_sign
import numpy as np
...
class ContactAttention(nn.Module):
    ...
    def forward(self, prior, seq, state, mask=None):
        """
        prior: L*L*1
        seq: L*4
        state: L*L
        ...
        if mask != None:
            return contact.view(-1, self.L, self.L) * mask
        else:
...
class ContactAttention_simple_fix_PE(ContactAttention_simple):
    ...
    def forward(self, pe, seq, state, mask=None):
        """
        prior: L*L*1
        seq: L*4
        state: L*L
